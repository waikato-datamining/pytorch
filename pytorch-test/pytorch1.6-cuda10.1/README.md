...
* Launch docker container and execute the script
  ...
  docker run \
    ...
    -it harbor.cms.waikato.ac.nz/public/pytorch/pytorchtest:pytorch1.6-cuda10.1 \
    /usr/bin/pytorchtest
  ```
...
* Launch docker container and execute the script
  ...
  docker run \
    ...
    --shm-size 8G \
    -v `pwd`/data:/opt/pytorchtest/data \
    -it waikatodatamining/pytorchtest:pytorch1.6-cuda10.1 \
    /usr/bin/pytorchtest
  ```
...
```bash
docker build -t pytorchtest:pytorch1.6-cuda10.1 .
```
...
* Tag
  ...
  ```bash
  docker tag \
    pytorchtest:pytorch1.6-cuda10.1 \
    harbor.cms.waikato.ac.nz/public/pytorch/pytorchtest:pytorch1.6-cuda10.1
  ```
  
* Push

  ```bash
  docker push harbor.cms.waikato.ac.nz/public/pytorch/pytorchtest:pytorch1.6-cuda10.1
  ```
...
* Tag
  ...
  docker tag \
    pytorchtest:pytorch1.6-cuda10.1 \
    waikatodatamining/pytorchtest:pytorch1.6-cuda10.1
  ```
  
* Push

  ```bash
  docker push waikatodatamining/pytorchtest:pytorch1.6-cuda10.1
  ```
  If error "no basic auth credentials" occurs, then run (enter username/password when prompted):
