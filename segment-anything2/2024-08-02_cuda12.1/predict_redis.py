...
def main(args=None):
    ...
    :type args: list
    """
    parser = create_parser('SAM2 - Prediction (Redis)', prog="sam2_predict_redis", prefix="redis_")
    ...
    # load model
    if parsed.verbose:
        print("Loading model/type: %s/%s" % (parsed.model, parsed.config))
    model_instance = load_model(parsed.model, parsed.config, parsed.device)

    config = Container()
    config.model = model_instance
    config.model_config = parsed.config
