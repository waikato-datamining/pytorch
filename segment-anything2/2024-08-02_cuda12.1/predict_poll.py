...
def process_image(fname, output_dir, poller):
    ...
    try:
        ...
        opex.meta = {
            "segmenter": {
                "type": "sam",
                "model_config": poller.params.model_config,
...
def predict_on_images(model_file, model_config, device, input_dir, output_dir, tmp_dir=None,
    ...
    :param model_file: the checkpoint file to use
    :param model_file: str
    :param model_config: the model config file to use
    ...
    """
    if verbose:
        print("Loading model/type: %s/%s" % (model_file, model_config))
    model_instance = load_model(model_file, model_config, device)
    ...
    poller.params.model_config = model_config
    poller.params.model_file = model_file
    poller.poll()
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(
        description="SAM2 - Prediction (file-polling)",
        prog="sam2_predict_poll",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--model', metavar="FILE", type=str, required=True, help='The SAM2 checkpoint to use.')
    parser.add_argument('--config', metavar="FILE", type=str, required=True, help='The config file to use .')
    ...
    predict_on_images(parsed.model, parsed.config, parsed.device, parsed.prediction_in, parsed.prediction_out, tmp_dir=parsed.prediction_tmp,
