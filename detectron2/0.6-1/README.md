...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --gpus=all --shm-size 8G --net=host \
    ...
    -it public.aml-repo.cms.waikato.ac.nz:443/pytorch/detectron2:0.6-1
  ```
...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --gpus=all --shm-size 8G --net=host \
    -v /local/dir:/container/dir \
    -it waikatodatamining/detectron2:0.6-1
  ```

### Build local image

* Build the image from Docker file (from within /path_to/detectron2/0.6-1)
...
```bash
docker build -t detectron2:0.6-1 .
```
...
* Tag
  ...
  docker tag \
    detectron2:0.6-1 \
    public-push.aml-repo.cms.waikato.ac.nz:443/pytorch/detectron2:0.6-1
  ```
  
* Push

  ```bash
  docker push public-push.aml-repo.cms.waikato.ac.nz:443/pytorch/detectron2:0.6-1
  ```
...
* Tag
  ...
  docker tag \
    detectron2:0.6-1 \
    waikatodatamining/detectron2:0.6-1
  ```
  
* Push

  ```bash
  docker push waikatodatamining/detectron2:0.6-1
  ```
  If error "no basic auth credentials" occurs, then run (enter username/password when prompted):
